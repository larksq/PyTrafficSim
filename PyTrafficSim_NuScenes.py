...
class IntersectionSim:
    def __init__(self, win, map_cls=DefaultMap_4ways.Map, scale=15,
                 frame_rate=30.0, window_h=1000, window_w=1000, extend=25, trajectory=True, spawn_vertical_margin=5,
        ...
        self.speed_random_range = 0.05
        # self.decision_cache = []
        # [[[agent.x, agent.y, agent.yaw, agent.next_action], ... (all agents)], ... (all timeframes)]
        # save next action of each agent at each time step for system usage
    ...
    def random_spawn_agent(self, generate_poly=True):
        speed_random_range = self.speed_random_range
        ...
        if generate_poly:
            self.generate_polygon_for_agent(spawned_agent)

        self.agents.append(spawned_agent)
        return spawned_agent

    def generate_polygon_for_agent(self, spawned_agent):
        # draw
        agent_w = spawned_agent.width
        agent_l = spawned_agent.length
        pt1, pt2, pt3, pt4 = generate_contour_pts((spawned_agent.x, spawned_agent.y),
                                                  agent_w, agent_l,
                                                  normalize_angle(spawned_agent.yaw + 0.5 * math.pi))
    ...
    def speculate(self):
        ...
        for checking_agent in self.agents:
            if len(checking_agent.trajectory) > 1:
                ...
                if checking_agent.agent_polys is not None and new_color is not None:
                    checking_agent.agent_polys.setFill(new_color)
                next_actions.append([checking_agent.next_action, new_color])
        return next_actions


    def check_collision(self, draw=True):
    ...
    def update(self):
        ...
        if self.simulate_dynamics:
            ...
            for i, agent in enumerate(self.agents):
                ...
                self.model.update(agent=agent, throttle=throttle, delta=delta)
    ...
    def save_current_pos(self):
        current_time_frame = []
        for agent in self.agents:
            current_time_frame.append([agent.x, agent.y, agent.yaw, agent.next_action])
        self.decision_cache.append(current_time_frame)
...
class TrafficLights:
    ...
    def init_traffic_lights(self, win, drawing=True):
    ...
    def update_traffic_lights(self, win, drawing=True):
    ...
    def check_flashing(self, win, drawing=True):
        if not drawing:
            return
        if self.init_drawn:
...
class SimWithPreComputedData(IntersectionSim):
    decision_cache = []
    # [[spawn_agent, [[index, ["next_action", "new_color"]], ..], finished_index], ...] for each agent
    # save next action of each agent at each time step for system usage
    # use same agent and same action at simulation to show animation

    def loop(self, agents_per_second=5, frame_rate=30, running_time=10):
        # running_time = running_time # seconds
        new_agent = self.random_spawn_agent(generate_poly=False)
        self.decision_cache.append([new_agent, [], None])
        self.frame_rate = frame_rate
        last_time = 0
        last_spawn = 0
        init_time = time.time()
        ...
        spawn_freq = agents_per_second  # n agents per second
        # for i in range(0, 300):
        self.traffic_lights.init_traffic_lights(win=self.win, drawing=False)
        # message_p = Text(Point(self.window_w / 2, self.window_h / 2), "Computing ... Please wait")
        # message_p.setTextColor("pink")
        # message_p.setSize(22)
        # message_p.draw(graphwin=self.win)
        ...
        while counter < running_time * self.frame_rate:
            ...
            if current_time - last_spawn > 1.0 / spawn_freq:
                ...
                last_spawn = current_time
            if current_time - last_time > 1.0 / self.frame_rate:
                next_actions = self.speculate()
                # next_actions: [[next_action, new_color], ...] for each agent
                if len(next_actions) > 0:
                    self.update_cache_with_actions(next_actions, counter)

                self.update()
                self.update_finished_agents()
                # update(100)
                last_time = current_time
                counter += 1
                self.check_collision(draw=False)
                self.clear_finished_agents()
            current_time += 1.0 / self.frame_rate
        ...
        system_log = "Simulation Finished\n\nCrashed: {}\nTotal traffic: {}\nSimulation time: {}".format(
                self.crash_time, self.finished_agent, str(running_time))
        print("Verbose Running Result: \n" + system_log)

    def update_cache_with_actions(self, next_actions, current_idx):
        for i, agent in enumerate(self.agents):
            for j, cache_list in enumerate(self.decision_cache):
                cache_agent = cache_list[0]
                if cache_agent == agent:
                    # print("test: ", cache_list, next_actions)
                    if len(cache_list[1]) < 1:
                        if i < len(next_actions):
                            # already speculated
                            cache_list[1].append([current_idx, next_actions[i]])
                    elif cache_list[1][-1][1][0] == next_actions[i][0]:
                        break
                    else:
                        if i < len(next_actions):
                            # already speculated
                        ...
                        break
    def update(self):
        self.traffic_lights.update_traffic_lights(self.win, drawing=False)
        self.traffic_lights.check_flashing(self.win, drawing=False)
        if self.simulate_dynamics:
            # follow the dynamics: Not supported
            pass
        else:
            # follow trajectory
            for i, agent in enumerate(self.agents):
